...
export function buildGeneratedDifferenceAnswer(parsed, recap, previousRecap) {
  ...
  }).filter(row => !parsed.roomName || normalize(row.name) === normalize(parsed.roomName))
  ...
  const a = total(before.recap), b = total(after.recap);
  const deltaText = value => value === 0 ? 'tetap' : `${value > 0 ? 'naik' : 'turun'} ${format(Math.abs(value))} kg`;
  const changed = changes.filter(row => row.delta !== 0);
