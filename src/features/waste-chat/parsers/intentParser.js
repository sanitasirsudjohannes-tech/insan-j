export function detectWasteIntent(text, { type, types, roomName }) {
  ...
  if (/(?:berapa|jumlah|banyaknya|total).*(?:ruang|ruangan)(?:.*(?:input|diinput|tercatat|mencatat|hari|tanggal|tgl))?|(?:ruang|ruangan).*(?:yang\s+)?(?:input|diinput|tercatat|mencatat).*(?:berapa|jumlah|banyaknya|total)/i.test(text)) return 'room_input_count';
