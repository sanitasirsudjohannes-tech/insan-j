...
const statusLabel = status => status === 'memenuhi'
  ? 'memenuhi'
  : status === 'tidak_memenuhi'
    ? 'tidak memenuhi'
    : 'belum dinilai';

const locationName = record => record.water_type === 'clean'
  ? record.water_clean_locations?.name
  : record.sample_point;

const normalizeRecord = record => ({
  ...record,
  parameters: (Array.isArray(record.parameters) ? record.parameters : []).map(item => ({
    ...item,
    parameter: record.water_type === 'clean' && /^coliform$/i.test(String(item.parameter).trim())
      ? 'Total coliform'
      : item.parameter,
...
const parameterText = parameters => parameters
  .map(item => `${item.parameter}: ${item.result}${item.unit ? ` ${item.unit}` : ''}${item.standard ? ` (baku mutu ${item.standard}${item.regulation ? `; rujukan ${item.regulation}` : ''})` : ''} — ${statusLabel(item.status)}`)
  .join('; ');

const recordLine = record =>
  `${formatDate(record.sampled_at)} — ${locationName(record) || '-'}: ${parameterText(record.parameters) || 'parameter tidak tersedia'}`;

export function summarizeWaterRecords(rawRecords = [], reportType) {
  ...
  const records = rawRecords.filter(record => record.water_type === waterType).map(normalizeRecord);
  const nonCompliant = records.flatMap(record => record.parameters
  ...
  const locations = [...new Set(records.map(locationName).filter(Boolean))];
  const common = {
    totalExaminations: records.length,
    totalParameters: records.reduce((total, record) => total + record.parameters.length, 0),
    compliantParameters: records.reduce((total, record) =>
      total + record.parameters.filter(item => item.status === 'memenuhi').length, 0),
    nonCompliantParameters: nonCompliant.length,
    unassessedParameters: records.reduce((total, record) =>
      total + record.parameters.filter(item => !['memenuhi', 'tidak_memenuhi'].includes(item.status)).length, 0),
    locations,
    records,
    nonCompliant,
  };
      ...
      facts: {
        ...
        compliance: nonCompliant.length
          ...
          : common.unassessedParameters
            ? `${common.unassessedParameters} parameter belum dinilai.`
            : 'Seluruh parameter dengan hasil dan baku mutu numerik berstatus memenuhi; verifikasi terhadap laporan lab tetap diperlukan.',
      },
