...
export default function useRuanganData() {
  ...
  const setFilterDate = useCallback((value) => {
    ...
    else setFilterMonthState(current => current || getCurrentMonth());
  }, []);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      let dbData = [], count = 0;
      const effectiveMonth = filterMonth || (!filterDate ? getCurrentMonth() : '');
      ...
      const mergedData = [...unsynced, ...dbData.filter(d => !hiddenServerIds.has(String(d.id)))].sort(compareWasteRows);
  ...
  useEffect(() => {
    fetchData(); const h = () => fetchData(); let queueRefreshTimer;
    const handleQueueChange = event => {
      ...
      window.clearTimeout(queueRefreshTimer); queueRefreshTimer = window.setTimeout(h, 180);
    };
    window.addEventListener('offline-queue-changed', handleQueueChange); window.addEventListener('offline-sync-finished', handleQueueChange); window.addEventListener('offline', h);
    const handleVisibility = () => { if (document.visibilityState === 'visible') h(); };
    document.addEventListener('visibilitychange', handleVisibility);
    const { data: { subscription } } = supabase.auth.onAuthStateChange(event => { if (['INITIAL_SESSION', 'SIGNED_IN', 'TOKEN_REFRESHED'].includes(event)) h(); });
    return () => {
      window.clearTimeout(queueRefreshTimer); window.removeEventListener('offline-queue-changed', handleQueueChange); window.removeEventListener('offline-sync-finished', handleQueueChange); window.removeEventListener('offline', h); document.removeEventListener('visibilitychange', handleVisibility); subscription?.unsubscribe();
    };
  }, [fetchData]);
  return { data, loading, page, setPage, totalData, offlineQueueCount, filterMonth, setFilterMonth, fetchData, ruanganList, filterRuangan, setFilterRuangan, filterDate, setFilterDate, setData, setTotalData, setOfflineQueueCount };
}
