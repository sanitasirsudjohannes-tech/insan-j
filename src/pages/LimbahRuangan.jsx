...
export default function LimbahRuangan({ embedded = false }) {
  ...
  const handleExportExcel = async () => {
    const { value: fv } = await MySwal.fire({ title:'Export Data Limbah Per Ruangan', html:`<div class="text-left space-y-3"><div><label class="block text-xs font-bold text-gray-600 mb-1">Pilih Bulan & Tahun:</label><input id="swal-export-month" type="month" class="swal2-input w-full m-0" value="${new Date().toISOString().slice(0,7)}"></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Filter Ruangan (Opsional):</label><select id="swal-export-ruangan" class="swal2-select w-full m-0"><option value="">-- Semua Ruangan --</option>${ruanganList.map(r=>`<option value="${r}">${r}</option>`).join('')}</select></div></div>`, focusConfirm:false, showCancelButton:true, confirmButtonText:'<i class="fas fa-file-excel mr-2"></i>Export Excel', cancelButtonText:'Batal', preConfirm:()=>({ month:document.getElementById('swal-export-month').value, ruangan:document.getElementById('swal-export-ruangan').value }) });
  ...
  const handlePrint = async () => {
    ...
    const {value:fv}=await MySwal.fire({ title:'Cetak Laporan Limbah Ruangan', html:`<div style="text-align:left"><p style="font-size:13px;color:#6b7280;margin-bottom:12px">Pilih periode dan ruangan yang ingin dicetak.</p><label style="display:block;font-size:12px;font-weight:700;color:#374151;margin-bottom:5px">Bulan & Tahun</label><input id="swal-print-month" type="month" class="swal2-input" style="width:100%;margin:0 0 14px 0" value="${currentMonth}"/><label style="display:block;font-size:12px;font-weight:700;color:#374151;margin-bottom:5px">Ruangan</label><select id="swal-print-ruangan" class="swal2-select" style="width:100%;margin:0"><option value="">-- Semua Ruangan --</option>${ruanganList.map(r=>`<option value="${r}">${r}</option>`).join('')}</select></div>`, focusConfirm:false, showCancelButton:true, confirmButtonText:'<i class="fas fa-print"></i> Cetak', cancelButtonText:'Batal', confirmButtonColor:'#2563eb', preConfirm:()=>{ const mi=document.getElementById('swal-print-month'); if(!mi?.value){ Swal.showValidationMessage('Silakan pilih bulan terlebih dahulu.'); return false; } return { month:mi.value, ruangan:document.getElementById('swal-print-ruangan')?.value||'' }; } });
    if (!fv) return;
    const {month:sel,ruangan:selR}=fv;
    const [y,m]=sel.split('-'); const s=`${y}-${m}-01`,en=`${y}-${m}-${String(new Date(+y,+m,0).getDate()).padStart(2,'0')}`;
    try {
