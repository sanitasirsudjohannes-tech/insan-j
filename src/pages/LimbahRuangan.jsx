...
export default function LimbahRuangan() {
  ...
  return (
    <AppLayout title="Limbah Per Ruangan">
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    list="ruanganDataList"
                    name="ruangan"
                    value={formData.ruangan}
                    onChange={handleInputChange}
                    placeholder="-- Ketik atau pilih ruangan --"
                    required
                    className="w-full border border-gray-300 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none text-sm bg-white"
                  />
                  <datalist id="ruanganDataList">
                    {ruanganList.map(r => <option key={r} value={r} />)}
                  </datalist>
                </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-100 text-gray-700 text-xs uppercase tracking-wider border-b">
                  ...
                  <th className="px-4 py-3.5 font-bold">Ruangan</th>
                  <th className="px-4 py-3.5 font-bold text-right">Infeksius</th>
                  <th className="px-4 py-3.5 font-bold text-right">Jarum</th>
                  <th className="px-4 py-3.5 font-bold text-right">Botol</th>
                  <th className="px-4 py-3.5 font-bold text-right">Sitotoksik</th>
                  <th className="px-4 py-3.5 font-bold text-right">Total</th>
                  <th className="px-4 py-3.5 font-bold">Petugas</th>
                ...
                {loading ? (
                  <tr>
                    <td colSpan="10" className="text-center py-10">
                      ...
                      <span className="text-gray-500 text-xs font-semibold">Memuat data limbah ruangan...</span>
                    </td>
                  </tr>
                ) : data.length === 0 ? (
                  <tr>
                    <td colSpan="10" className="text-center py-12 text-gray-400">
                      <i className="fas fa-inbox text-4xl mb-3 block opacity-40"></i>
                      Belum ada data limbah ruangan.
                    </td>
                  </tr>
                ) : (
                  data.map((item, idx) => {
                    ...
                    return (
                      <tr
                        key={item.id}
                        className={item.isOffline ? "bg-amber-50/70 hover:bg-amber-100/70 border-l-4 border-l-amber-500 transition-colors" : "hover:bg-emerald-50/40 transition-colors"}
                      >
                        <td className="px-4 py-3 text-gray-400 text-xs font-medium">{rowNo}</td>
                        ...
                        <td className="px-4 py-3 font-bold text-emerald-700">
                          <span className="inline-block bg-emerald-100 text-emerald-800 text-xs px-2.5 py-1 rounded-lg">
                            {item.ruangan}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-right text-red-600 font-semibold">{inf.toFixed(2)}</td>
                        <td className="px-4 py-3 text-right text-orange-600 font-semibold">{jar.toFixed(2)}</td>
                        <td className="px-4 py-3 text-right text-blue-600 font-semibold">{bot.toFixed(2)}</td>
                        <td className="px-4 py-3 text-right text-purple-600 font-semibold">{sit.toFixed(2)}</td>
                        <td className="px-4 py-3 text-right font-black text-slate-800">{total.toFixed(2)} Kg</td>
                        <td className="px-4 py-3 text-gray-600 text-xs">{item.petugas || '-'}</td>
                        <td className="px-4 py-3 text-center whitespace-nowrap">
                          <button
                            onClick={() => handleEdit(item)}
                            className="bg-blue-100 text-blue-600 hover:bg-blue-200 px-2.5 py-1.5 rounded-lg mx-1 transition active:scale-95"
                            title="Edit Data"
                          >
                            <i className="fas fa-edit"></i>
                          </button>
                          <button
                            ...
                            className="bg-red-100 text-red-600 hover:bg-red-200 px-2.5 py-1.5 rounded-lg mx-1 transition active:scale-95"
                            title="Hapus Data"
                          >
