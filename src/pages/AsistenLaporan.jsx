...
async function createWordFile(draft, reportType, chartImages = []) {
  const filename = `Draft_${REPORT_TYPES[reportType]?.shortLabel || 'Laporan'}_${getLocalDateString()}.docx`
    .replace(/\s+/g, '_')
    .replace(/[^a-zA-Z0-9_.-]/g, '');
  const blob = await buildDocxBlob(draft, chartImages);
  return new File([blob], filename, { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
}

async function downloadWord(draft, reportType, chartImages = []) {
  const file = await createWordFile(draft, reportType, chartImages);
