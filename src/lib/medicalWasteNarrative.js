...
export function buildMedicalWasteAnalysis(facts = {}, analytics = null) {
  ...
  if (!analytics) {
    ...
    return `Selama periode pelaporan, timbulan limbah medis tercatat sebanyak ${format(generated)} kg dan pengangkutan sebanyak ${format(transported)} kg. Setelah memperhitungkan sisa limbah awal periode sebesar ${format(opening)} kg, sisa limbah akhir periode tercatat sebesar ${format(remaining)} kg.\n\nJenis limbah yang paling dominan adalah ${dominant[0]} sebanyak ${format(dominant[1])} kg. Data pembanding periode sebelumnya belum tersedia sehingga kecenderungan kenaikan atau penurunan belum dapat dinilai.`;
  }
  ...
  const balanceText = analytics.performance.negativeBalance
    ? `Terdapat ketidaksesuaian perhitungan sebesar ${format(Math.abs(remaining))} kg sehingga pencatatan timbulan, sisa awal, dan pengangkutan perlu diverifikasi.`
    : analytics.performance.accumulationIncreased
      ? `Sisa limbah akhir periode meningkat ${format(analytics.changes.remainingKg)} kg dibandingkan sisa awal, yang menunjukkan adanya penambahan penumpukan.`
      : 'Sisa limbah akhir periode tidak meningkat dibandingkan sisa awal sehingga pengangkutan mampu mengimbangi timbulan pada periode ini.';
...
export function buildMedicalWasteRecommendations(facts = {}, analytics = null) {
  ...
  const remaining = number(facts.remainingKg);
  if (remaining < 0 || analytics?.performance.negativeBalance) recommendations.push('Verifikasi kesesuaian sisa limbah awal, timbulan, dan dokumen pengangkutan karena terdapat ketidaksesuaian perhitungan.');
  else if (analytics?.performance.accumulationIncreased) recommendations.push('Evaluasi jadwal dan kapasitas pengangkutan karena sisa limbah akhir periode meningkat dibandingkan sisa awal.');
  else recommendations.push('Pertahankan jadwal pengangkutan dan pemantauan sisa limbah tersimpan agar tidak terjadi penumpukan pada periode berikutnya.');
...
export function buildMedicalWasteConclusion(facts = {}, analytics = null) {
  ...
  const condition = remaining < 0 ? 'Ketidaksesuaian perhitungan menunjukkan data masih perlu diverifikasi sebelum laporan ditetapkan.' : analytics?.performance.accumulationIncreased ? 'Sisa limbah akhir periode meningkat sehingga pengendalian penumpukan perlu menjadi perhatian.' : 'Pengangkutan mampu menjaga sisa limbah agar tidak meningkat dibandingkan awal periode.';
  return `Pengelolaan limbah medis mencatat timbulan ${format(facts.totalGeneratedKg)} kg, pengangkutan ${format(facts.totalTransportedKg)} kg, dan sisa limbah akhir periode ${format(remaining)} kg. ${trend} ${condition}`;
}
