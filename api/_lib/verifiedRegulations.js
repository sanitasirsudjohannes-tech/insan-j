...
export function findVerifiedRegulations(question, limit = 5) {
  ...
  return REGULATIONS.map(item => ({
    ...item,
    score: item.keywords.reduce((score, keyword) => score + (words.includes(keyword) ? 1 : 0), 0),
  })).filter(item => item.score > 0).sort((left, right) => right.score - left.score).slice(0, limit).map(({ keywords, score, ...item }) => item);
}
