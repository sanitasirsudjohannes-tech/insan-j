...
async function callGemini(question, contextPeriod) {
  ...
  const now = witaNow();
  const prompt = `Tanggal acuan WITA: tahun ${now.year}, bulan ${now.month}. Pahami pertanyaan pengguna tentang data limbah medis INSAN-J, termasuk bahasa sehari-hari, singkatan, dan salah ketik ringan. Kembalikan JSON saja dengan properti: intent, year, month, day, startDate, endDate, typeKey, typeKeys, roomName, inferredYear. intent hanya boleh waste_summary, remaining, opening_balance, available_total, generated, transported, transport_dates, type_dates, transport_coverage, average, dominant_type, type_breakdown, top_rooms, bottom_room, type_rooms, room_total, room_type_total, peak_day, active_days, comparison, type_total, atau unknown. Gunakan transport_dates jika pengguna menanyakan tanggal atau hari apa saja pengangkutan dilakukan. Gunakan type_dates jika pengguna menanyakan tanggal atau hari apa saja suatu jenis limbah tercatat. Gunakan type_rooms jika pengguna meminta daftar ruangan yang memiliki atau menghasilkan jenis limbah tertentu; jangan salin frasa seperti “yang ada limbah” sebagai roomName. Gunakan waste_summary untuk permintaan umum seperti rincian limbah, data limbah, ringkasan, rekap, atau gambaran keseluruhan. Gunakan room_total untuk jumlah seluruh jenis dari satu ruangan dan room_type_total untuk satu jenis limbah dari satu ruangan; salin nama ruang ke roomName tanpa mengarang nama. Untuk rentang tanggal isi startDate dan endDate dalam YYYY-MM-DD. Untuk satu tanggal isi year, month, day. Untuk bulan isi year dan month dengan day null. Untuk satu tahun isi year dengan month dan day null. typeKey hanya infectiousKg, sharpsKg, bottleKg, cytotoxicKg, atau null; typeKeys boleh memuat beberapa jenis untuk type_breakdown. Jika pengguna merujuk “tanggal tersebut” atau “periode itu”, gunakan konteks periode sebelumnya berikut: ${JSON.stringify(contextPeriod)}. Jika periode tidak disebutkan, gunakan bulan dan tahun acuan serta inferredYear true. Jangan menjawab angka dan jangan menambah fakta. Pertanyaan: ${JSON.stringify(question)}`;
